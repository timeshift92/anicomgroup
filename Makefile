...
install: ## Install all app dependencies
	$(dc_bin) run -e STARTUP_WAIT_FOR_SERVICES=false $(RUN_APP_ARGS) composer update --no-interaction --ansi --prefer-dist
...
up: ## Create and start containers
	...
	$(call print_block, 'Navigate your browser to         ⇒ http://localhost:$(FRONTEND_PORT)')
	$(call print_block, 'Redis Manager (Redis Web UI)     ⇒ http://localhost:$(REDIS_MANAGER_PORT)')
